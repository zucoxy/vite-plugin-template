...
/**
 * 感谢大佬提供的模板
 * @link https://github.com/jeddygong/vite-templates/blob/master/vite-plugin-template/src/index.ts
 */
export function vitePluginTemplate(): Plugin {
  return {
    // 插件名称
    ...
    // vite 独有的钩子：可以在 vite 被解析之前修改 vite 的相关配置。钩子接收原始用户配置 config 和一个描述配置环境的变量env
    config(config, { command, mode }) {},

    // vite 独有的钩子：在解析 vite 配置后调用。使用这个钩子读取和存储最终解析的配置。当插件需要根据运行的命令做一些不同的事情时，它很有用。
    configResolved(resolvedConfig) {},

    // vite 独有的钩子：主要用来配置开发服务器，为 dev-server (connect 应用程序) 添加自定义的中间件
